...
//           easy reference
    ...
    public double returnPizzaCost(char size, int numberOfToppings) {
        ...
        if (numberOfToppings <=3){
            costOfPizza += (UNDER_3_TOPPINGS * numberOfToppings);
        } else {
            costOfPizza += (OVER_3_TOPPINGS * numberOfToppings);
        }
        return costOfPizza;
        // 3. set the cost based on the size of pizza
        // 4. does it have more or less then 3 toppings
